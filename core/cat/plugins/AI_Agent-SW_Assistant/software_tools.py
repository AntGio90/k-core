...
PHASE = "software"
WP_ID = "WP-SW-SRS"
TITLE = "SW Safety Requirements Specification"
...
REVIEW_CHECKLIST = [
    "Derived from approved Technical Safety Concept",
    "Software safety requirements are complete",
    "Interfaces and dependencies are identified",
    "Safety mechanisms are described",
...
def _build_revision_prompt(change_requests, content):
    ...
    return (
        "You are a functional safety engineer. Update the SW Safety Requirements Specification markdown to apply the change requests. "
...
@tool(return_direct=True, examples=["generate sw srs"])
def generate_sw_srs(tool_input, cat):
    """Generate a SW Safety Requirements Specification draft. Input: JSON with item_name, author, and optional sections."""
    ...
    upstream, missing_upstream = store.resolve_upstream(project_slug, UPSTREAM_SW)
    ...
    template_path = Path(__file__).resolve().parent / "templates" / "sw_srs.md"
    body_template = template_renderer.load_template(template_path)

    context = {
        "software_scope": utils.normalize_text(data.get("software_scope"), f"Item: {fields['item_name']}"),
        "sw_safety_requirements": template_renderer.format_bullets(data.get("sw_safety_requirements")),
        "interfaces": template_renderer.format_bullets(data.get("interfaces")),
        "safety_mechanisms": template_renderer.format_bullets(data.get("safety_mechanisms")),
        "architecture_constraints": template_renderer.format_bullets(data.get("architecture_constraints")),
...
@tool(return_direct=True, examples=["request review sw srs {\"artifact_id\": \"wp_sw_srs-...\"}"])
def request_review_sw_srs(tool_input, cat):
    """Request review for a SW SRS. Input: {"artifact_id": "...", "reviewer": "..."}."""
    data = utils.parse_tool_input(tool_input)
...
@tool(return_direct=True, examples=["request changes sw srs {\"artifact_id\": \"wp_sw_srs-...\", \"change_requests\": [""..."""]}"])
def request_changes_sw_srs(tool_input, cat):
    """Record change requests for a SW SRS. Input: {"artifact_id": "...", "change_requests": [...]}"""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
...
@tool(return_direct=True, examples=["revise sw srs {\"artifact_id\": \"wp_sw_srs-...\", \"change_requests\": [""..."""]}"])
def revise_sw_srs(tool_input, cat):
    """Revise a SW SRS. Input: {"artifact_id": "...", "change_requests": [...], "revised_document": "..."}."""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
        return template_renderer.render_missing_inputs(["artifact_id"])
    ...
    if revised_document:
        ...
        if change_requests:
            ...
            try:
                ...
                log.warning(f"SW SRS revision LLM failed: {exc}")
...
@tool(return_direct=True, examples=["approve sw srs {\"artifact_id\": \"wp_sw_srs-...\", \"reviewer\": ""name""}"])
def approve_sw_srs(tool_input, cat):
    """Approve a SW SRS. Input: {"artifact_id": "...", "reviewer": "...", "notes": "..."}."""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
