...
PHASE = "hardware"
WP_ID = "WP-HW-SRS"
TITLE = "HW Safety Requirements Specification"
...
REVIEW_CHECKLIST = [
    ...
    "Hardware safety requirements are complete",
...
def _build_revision_prompt(change_requests, content):
    ...
    return (
        "You are a functional safety engineer. Update the HW Safety Requirements Specification markdown to apply the change requests. "
...
@tool(return_direct=True, examples=["generate hw srs"])
def generate_hw_srs(tool_input, cat):
    """Generate a HW Safety Requirements Specification draft. Input: JSON with item_name, author, and optional sections."""
    ...
    upstream, missing_upstream = store.resolve_upstream(project_slug, UPSTREAM_HW)
    ...
    template_path = Path(__file__).resolve().parent / "templates" / "hw_srs.md"
    body_template = template_renderer.load_template(template_path)

    context = {
        "hardware_scope": utils.normalize_text(data.get("hardware_scope"), f"Item: {fields['item_name']}"),
        "hw_safety_requirements": template_renderer.format_bullets(data.get("hw_safety_requirements")),
        ...
        "hardware_metrics": template_renderer.format_bullets(data.get("hardware_metrics")),
...
@tool(return_direct=True, examples=["request review hw srs {\"artifact_id\": \"wp_hw_srs-...\"}"])
def request_review_hw_srs(tool_input, cat):
    """Request review for a HW SRS. Input: {"artifact_id": "...", "reviewer": "..."}."""
    data = utils.parse_tool_input(tool_input)
...
@tool(return_direct=True, examples=["request changes hw srs {\"artifact_id\": \"wp_hw_srs-...\", \"change_requests\": [""..."""]}"])
def request_changes_hw_srs(tool_input, cat):
    """Record change requests for a HW SRS. Input: {"artifact_id": "...", "change_requests": [...]}"""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
...
@tool(return_direct=True, examples=["revise hw srs {\"artifact_id\": \"wp_hw_srs-...\", \"change_requests\": [""..."""]}"])
def revise_hw_srs(tool_input, cat):
    """Revise a HW SRS. Input: {"artifact_id": "...", "change_requests": [...], "revised_document": "..."}."""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
        return template_renderer.render_missing_inputs(["artifact_id"])
    ...
    if revised_document:
        ...
        if change_requests:
            ...
            try:
                ...
                log.warning(f"HW SRS revision LLM failed: {exc}")
...
@tool(return_direct=True, examples=["approve hw srs {\"artifact_id\": \"wp_hw_srs-...\", \"reviewer\": ""name""}"])
def approve_hw_srs(tool_input, cat):
    """Approve a HW SRS. Input: {"artifact_id": "...", "reviewer": "...", "notes": "..."}."""
    data = utils.parse_tool_input(tool_input)
    artifact_id = data.get("artifact_id") or data.get("text")
    if not artifact_id:
